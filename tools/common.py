...
var FT = (function(){
  ...
  function applySnapshot(snap){
    ...
    if(snap.holdings){
      ...
      snap.holdings.forEach(function(h){
        ...
        held[h.asset_id] = { n: h.name, shares: Number(h.shares), avg: Number(h.avg_cost),
    ...
    if(snap.transactions){
      state.transactions = snap.transactions.map(function(t){
        return { type: t.type, asset: t.asset_id || t.label, shares: Number(t.shares) || 1,
  ...
  function push(fn, args, onResult, onRefused){
    ...
    db.call(fn, args).then(function(res){
      ...
      if(typeof showToast === 'function') showToast(error.message + ' Your balance has been re-checked.', 'error');
