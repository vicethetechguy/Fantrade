...
:root{
  ...
  --lime:#1800ad;--amber:#FF6A1F;--ink:#F4F6F1;--dim:#8B918A;--faint:#5A605B;--red:#FF5E5E;
...
.orb-a{width:780px;height:640px;top:-200px;left:50%;transform:translateX(-58%);background:radial-gradient(circle,rgba(24,0,173,.28),transparent 68%)}
.orb-b{width:620px;height:620px;top:48%;right:-240px;background:radial-gradient(circle,rgba(255,106,31,.18),transparent 68%)}
.orb-c{width:700px;height:560px;bottom:-280px;left:-200px;background:radial-gradient(circle,rgba(24,0,173,.14),transparent 70%)}
...
.btn-lime{background:var(--lime);color:#fff;box-shadow:var(--inset),0 14px 34px -14px rgba(24,0,173,.75)}
.btn-lime .cap{background:rgba(10,13,3,.14)}
.btn-lime:hover{background:#3311cc}
...
/* the clubs you own, switchable in one tap */
.clubrail{display:flex;gap:10px;margin-top:22px;overflow-x:auto;padding-bottom:6px;scrollbar-width:none}
.clubrail::-webkit-scrollbar{display:none}
.clubchip{flex:none;display:flex;align-items:center;gap:11px;padding:11px 16px 11px 12px;cursor:pointer;
  border:1px solid var(--hair);border-radius:16px;background:rgba(255,255,255,.03);box-shadow:var(--inset);
  font-family:Montserrat,sans-serif;text-align:left;transition:all .5s var(--ease)}
...
.clubchip .cn{font-size:13px;color:var(--ink);white-space:nowrap}
.clubchip .cd{font-family:'Montserrat', sans-serif;font-size:10px;color:var(--faint);white-space:nowrap;
  padding-left:11px;border-left:1px solid var(--hair)}
.clubchip.add .cd{border-left-color:rgba(24,0,173,.25)}
@media (max-width:560px){.clubchip .cd{display:none}}
...
.phead h1{font-size:clamp(40px,7vw,86px);font-weight:700;margin-top:26px}
...
.statbar b{font-family:'Montserrat', sans-serif;font-weight:400;color:var(--ink)}
...
.mrow.sel{background:rgba(24,0,173,.05)}
...
.t-sym{font-family:'Montserrat', sans-serif;font-size:13.5px}
...
.tick{font-family:'Montserrat', sans-serif;font-size:13px;transition:color .8s var(--ease)}
...
.tradebtn:hover{background:var(--lime);border-color:var(--lime);color:#fff}
...
.seg button[aria-pressed="true"]{background:var(--lime);color:#fff;box-shadow:var(--inset)}
...
.field input{border:0;background:transparent;color:var(--ink);text-align:right;outline:none;width:60%;
  font-family:'Montserrat', sans-serif;font-size:17px;font-variant-numeric:tabular-nums}
...
.quick button{flex:1;border:1px solid var(--hair);background:rgba(255,255,255,.03);color:var(--dim);border-radius:999px;
  padding:9px 0;font-family:'Montserrat', sans-serif;font-size:11px;cursor:pointer;transition:all .6s var(--ease);min-width:62px}
...
.line b{font-family:'Montserrat', sans-serif;font-weight:400;color:var(--ink)}
.supply{height:5px;border-radius:99px;background:rgba(255,255,255,.07);margin:16px 0 6px;overflow:hidden}
.supply i{display:block;height:100%;border-radius:99px;background:linear-gradient(90deg,#0f0075,var(--lime));box-shadow:0 0 16px rgba(24,0,173,.5)}

/* pitch */
.pitch{position:relative;padding:32px 26px;height:100%;
  background:radial-gradient(ellipse at 50% 0%,rgba(24,0,173,.07),transparent 62%),
...
.pitch-head .coach .player-face{width:38px;height:38px;border-radius:50%;overflow:hidden;display:block;border:1px solid rgba(255,106,31,.3);background:#171a18;box-shadow:0 5px 14px rgba(0,0,0,.3)}
.pitch-head .coach .player-face img{width:100%;height:100%;object-fit:cover;object-position:50% 18%;display:block}
...
.chip .nm{font-family:'Montserrat', sans-serif;font-size:12.5px;margin-top:4px}
.chip .player-face{width:44px;height:44px;border-radius:50%;overflow:hidden;display:block;margin:5px auto 4px;
  border:1px solid rgba(255,255,255,.14);background:#171a18;box-shadow:0 5px 14px rgba(0,0,0,.32)}
.chip .player-face img{width:100%;height:100%;display:block;object-fit:cover;object-position:50% 18%}
.chip.cap{border-color:rgba(24,0,173,.45);background:rgba(24,0,173,.07)}
...
.own-alert strong{font-family:'Montserrat', sans-serif;color:#ff9a9a;font-weight:400}
...
.mini .v{font-family:'Montserrat', sans-serif;font-weight:300;font-size:26px;margin-top:8px;letter-spacing:-.02em}
.mini .v.lime{color:var(--lime)}.mini .v.amber{color:var(--amber)}
.value-big{font-family:'Montserrat', sans-serif;font-weight:300;font-size:clamp(34px,3.6vw,46px);
...
.b-row b{color:var(--ink);font-weight:400;font-family:'Montserrat', sans-serif;font-variant-numeric:tabular-nums}
...
.tab[aria-selected="true"]{background:var(--lime);color:#20290A;border-color:var(--lime);box-shadow:var(--inset),0 16px 40px -18px rgba(24,0,173,.8)}
...
/* feature card */
.f h4{font-size:19px;font-weight:700;margin:18px 0 10px}
...
.ft-toast.success{border-color:rgba(24,0,173,.45)}
...
.ft-modal-box{background:var(--core);border:1px solid var(--hair);border-radius:24px;box-shadow:var(--ambient),var(--inset);max-width:540px;width:100%;max-height:90vh;overflow-y:auto;scrollbar-width:none;padding:32px;position:relative;transform:scale(.95);transition:transform .25s cubic-bezier(.16,1,.3,1)}
.ft-modal-box::-webkit-scrollbar{display:none}
...
.ft-modal-card .m-row b{color:var(--ink);font-family:'Montserrat', sans-serif;font-weight:400}
...
@media (max-width:768px){
  ...
  .overlay a{font-size:clamp(22px,7.5vw,34px)!important;font-weight:700!important;overflow-wrap:break-word;word-break:break-word;white-space:normal;line-height:1.25;padding:10px 0;border-bottom:1px solid rgba(255,255,255,.05)}
  ...
  .hero h1{font-size:clamp(26px,8.5vw,42px)!important;font-weight:700!important;letter-spacing:-.02em!important;overflow-wrap:break-word;word-break:break-word;margin-top:20px}
  .hero .lede{font-size:14.5px;margin-top:16px}
  .phead{padding:112px 0 40px}
  .phead h1{font-size:clamp(26px,8.5vw,42px)!important;font-weight:700!important;overflow-wrap:break-word;word-break:break-word;margin-top:18px}
  .phead .lede{font-size:14.5px;margin-top:16px}
  h2{font-size:clamp(23px,7vw,34px)!important;font-weight:700!important}
  .cta-sec h2{font-size:clamp(25px,7.8vw,38px)!important;font-weight:700!important}
  ...
  .chip .player-face{width:34px;height:34px;margin:4px auto 3px}
...
.nav-bell-badge{position:absolute;top:-4px;right:-4px;min-width:17px;height:17px;border-radius:999px;background:#1800ad;color:#fff;font-size:10px;font-weight:800;display:grid;place-items:center;padding:0 4px;box-shadow:0 0 10px rgba(24,0,173,.6);border:1.5px solid #0A0B0C;line-height:1;z-index:2}
.nav-bell-badge[hidden],.nav-bell-badge.hidden{display:none!important}
...
.menu .who span{display:block;font-family:'Montserrat', sans-serif;font-size:11px;color:var(--lime);margin-top:4px}
...
.tf .inp{display:flex;align-items:center;gap:12px;border:1px solid var(--hair);border-radius:14px;
  ...
  transition:border-color .5s var(--ease),background .5s var(--ease)}
.tf .inp:focus-within{border-color:var(--hair);background:rgba(255,255,255,.03);outline:none}
...
.tf.ok .inp{border-color:var(--hair);outline:none;box-shadow:none}
...
.checkrow input:checked+.box{background:var(--lime);border-color:var(--lime);color:#fff}
.checkrow input:focus-visible+.box{outline:none}
.checkrow a{color:var(--lime);border-bottom:1px solid rgba(24,0,173,.3)}
...
.auth-brand h1{font-size:clamp(34px,3.9vw,58px);font-weight:700;margin-top:24px}
...
.auth-alt a{color:var(--lime);border-bottom:1px solid rgba(24,0,173,.3);padding-bottom:1px}
...
.demo-note{margin-top:22px;border:1px dashed rgba(24,0,173,.3);border-radius:14px;padding:14px 16px;
  font-size:11.5px;color:var(--dim);font-weight:300;background:rgba(24,0,173,.035);line-height:1.6}
.demo-note b{font-family:'Montserrat', sans-serif;color:var(--lime);font-weight:400}
...
.ft-flash-up{animation:ftFlashUp 1.1s ease-out}.ft-flash-down{animation:ftFlashDown 1.1s ease-out}@keyframes ftFlashUp{0%{background-color:rgba(22,199,132,.22)}100%{background-color:transparent}}@keyframes ftFlashDown{0%{background-color:rgba(234,57,67,.22)}100%{background-color:transparent}}.ft-live-dot{display:inline-flex;align-items:center;gap:6px;font:600 11px/1 Montserrat,system-ui,sans-serif;color:#16c784;letter-spacing:.02em}.ft-live-dot::before{content:'';width:7px;height:7px;border-radius:50%;background:#16c784;box-shadow:0 0 0 0 rgba(22,199,132,.6);animation:ftPulse 1.8s infinite}html:not(.sim-on) .ft-live-dot{color:#8a8f86}html:not(.sim-on) .ft-live-dot::before{background:#8a8f86;animation:none}@keyframes ftPulse{0%{box-shadow:0 0 0 0 rgba(22,199,132,.55)}70%{box-shadow:0 0 0 7px rgba(22,199,132,0)}100%{box-shadow:0 0 0 0 rgba(22,199,132,0)}}@media (prefers-reduced-motion:reduce){.ft-flash-up,.ft-flash-down,.ft-live-dot::before{animation:none}}
...
.dr.you{background:rgba(24,0,173,.055);box-shadow:inset 2px 0 0 var(--lime)}
.dr.you:hover{background:rgba(24,0,173,.08)}
...
.tag.lime{background:rgba(24,0,173,.09);border-color:rgba(24,0,173,.28);color:var(--lime)}
...
.tgl[aria-pressed="true"]{background:rgba(24,0,173,.2);border-color:rgba(24,0,173,.5)}
.tgl[aria-pressed="true"] i{left:24px;background:var(--lime);box-shadow:0 0 14px rgba(24,0,173,.7)}
...
.setnav a.on{background:rgba(24,0,173,.08);border-color:rgba(24,0,173,.22);color:var(--lime)}
...
.fd.unread{background:rgba(24,0,173,.035)}
...
.fd.unread .tt::after{content:"";width:6px;height:6px;border-radius:99px;background:var(--lime);
  flex:none;box-shadow:0 0 10px rgba(24,0,173,.9)}
...
.fd .amt{font-family:'Montserrat', sans-serif;font-size:13.5px;flex:none;text-align:right;white-space:nowrap}
...
.pick[aria-pressed="true"]{border-color:var(--lime);background:rgba(24,0,173,.07)}
.pick .sym{font-family:'Montserrat', sans-serif;font-size:13px}
.pick .nm{font-size:11px;color:var(--faint);margin-top:4px}
.pick .px{font-family:'Montserrat', sans-serif;font-size:15px;margin-top:12px;font-weight:300}
...
.clock .u b{display:block;font-family:'Montserrat', sans-serif;font-size:22px;font-weight:300;letter-spacing:-.02em}
...
.app-head h1{font-size:clamp(32px,5vw,58px);font-weight:700;margin-top:20px}
...
/* ── wallet: balance hero ───────────────────────────── */
.bal-big{font-family:'Montserrat', sans-serif;font-weight:200;font-size:clamp(36px,5vw,66px);
...
.range button{border:0;background:transparent;color:var(--faint);border-radius:999px;padding:8px 18px;
  cursor:pointer;font-family:'Montserrat', sans-serif;font-size:11.5px;transition:all .5s var(--ease)}
.range button[aria-pressed="true"]{background:var(--lime);color:#fff;box-shadow:var(--inset)}
...
.acts4 button[aria-pressed="true"]{background:var(--lime);border-color:var(--lime);color:#fff;
  box-shadow:var(--inset),0 16px 38px -16px rgba(24,0,173,.8)}
...
.coin.lime{border-color:rgba(24,0,173,.35);background:rgba(24,0,173,.1);color:var(--lime)}
...
.arow .qt{font-family:'Montserrat', sans-serif;font-size:11px;color:var(--faint);margin-top:4px}
.arow .val{text-align:right;font-family:'Montserrat', sans-serif;font-size:15px;font-weight:300}
.arow .chg{text-align:right;font-family:'Montserrat', sans-serif;font-size:11.5px;margin-top:4px}
...
.addr{font-family:'Montserrat', sans-serif;font-size:12px;word-break:break-all;line-height:1.75;
...
.listening{display:inline-flex;align-items:center;gap:8px;border-radius:999px;padding:6px 13px;
  background:rgba(24,0,173,.09);border:1px solid rgba(24,0,173,.25);color:var(--lime);
...
.listening i{width:6px;height:6px;border-radius:99px;background:var(--lime);display:block;
  box-shadow:0 0 10px rgba(24,0,173,.9);animation:blip 1.9s var(--ease) infinite}
@keyframes blip{0%,100%{opacity:1}50%{opacity:.25}}

/* ── taskbar dock (full width at screen bottom) ─────── */
.topbar{padding:8px 8px 8px 22px}
.topbar .logo{margin-right:26px}
.taskbar{position:fixed;left:0;right:0;bottom:0;z-index:999;
  display:grid;grid-template-columns:repeat(5,1fr);align-items:center;
  width:100%;height:calc(60px + env(safe-area-inset-bottom, 0px));box-sizing:border-box;
  background:rgba(8,9,10,.98);backdrop-filter:blur(24px) saturate(160%);
  -webkit-backdrop-filter:blur(24px) saturate(160%);
  border-top:1px solid rgba(255,255,255,.08);box-shadow:0 -4px 24px rgba(0,0,0,.7);
  padding:0 max(0px, calc(50% - 340px)) env(safe-area-inset-bottom, 0px);border-radius:0;margin:0}
.taskbar a{display:flex;flex-direction:column;align-items:center;justify-content:center;
  gap:2px;height:58px;color:#767c82;text-decoration:none;
  transition:color .2s ease;position:relative}
.taskbar a .tb-icon-box{position:relative;display:flex;align-items:center;justify-content:center}
.taskbar a .ic{width:22px;height:22px;color:inherit;transition:transform .2s ease}
.taskbar a span.tb-label{font-family:Montserrat,sans-serif;font-size:10px;font-weight:500;
  letter-spacing:.02em;color:inherit;white-space:nowrap}
.taskbar a:hover{color:var(--ink)}
.taskbar a.on{color:var(--lime);font-weight:600}
.taskbar a.on .ic{color:var(--lime);transform:scale(1.05)}
.taskbar .tb-badge{position:absolute;top:-3px;right:-9px;background:#FF3B47;color:#fff;
  font-family:'Montserrat', sans-serif;font-size:9px;font-weight:700;
  min-width:14px;height:14px;border-radius:999px;display:flex;
  align-items:center;justify-content:center;padding:0 3px;
  border:1.5px solid #08090a}
.taskbar ~ footer{padding-bottom:calc(88px + env(safe-area-inset-bottom, 0px))}
.taskbar ~ .ft-toast-container{bottom:calc(76px + env(safe-area-inset-bottom, 0px))}

/* ── wallet: gradient balance card ──────────────────── */
.wcard{position:relative;border-radius:26px;padding:26px 24px 22px;overflow:hidden;
  background:linear-gradient(150deg,#4422dd 0%,#1800ad 44%,#0f0075 100%);color:#fff;
  box-shadow:0 26px 62px -22px rgba(24,0,173,.5),var(--inset)}
...
.wcard .amt{font-family:'Montserrat', sans-serif;font-weight:300;font-size:clamp(32px,4.4vw,52px);
...
.wcard .tag-id{position:absolute;top:26px;right:24px;font-family:'Montserrat', sans-serif;
...
.wacts button,.wacts a{display:flex;flex-direction:column;align-items:center;gap:9px;padding:14px 4px 12px;
  border-radius:18px;border:1px solid rgba(10,13,3,.16);background:rgba(10,13,3,.08);color:#fff;
...
.wacts button[aria-pressed="true"]{background:#0A0D03;color:var(--lime);border-color:#fff}
...
.qp .av.add{border-style:dashed;border-color:rgba(24,0,173,.42);color:var(--lime);
  background:rgba(24,0,173,.07)}
...
.trow .a{font-family:'Montserrat', sans-serif;font-size:13.5px;flex:none;white-space:nowrap}
...
.asset-head .px .v{font-family:'Montserrat', sans-serif;font-weight:300;font-size:clamp(26px,3vw,38px);
  letter-spacing:-.03em;line-height:1}
.asset-head .px .d{font-family:'Montserrat', sans-serif;font-size:13px;margin-top:8px}
...
.brow{position:relative;display:flex;justify-content:space-between;padding:7px 8px;
  font-family:'Montserrat', sans-serif;font-size:12px;border-radius:7px;overflow:hidden}
.brow i{position:absolute;top:0;bottom:0;right:0;display:block;border-radius:7px}
.brow span{position:relative;z-index:1}
.book .bid .brow i{background:rgba(24,0,173,.13)}
...
.spread{text-align:center;padding:12px 0;margin:6px 0;border-top:1px solid var(--hair);
  border-bottom:1px solid var(--hair);font-family:'Montserrat', sans-serif;font-size:12px;color:var(--faint)}
...
.sideseg button[aria-pressed="true"][data-s="buy"]{background:var(--lime);color:#fff}
...
.tr-row{display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;padding:9px 0;
  font-family:'Montserrat', sans-serif;font-size:12px;border-bottom:1px solid rgba(255,255,255,.04)}
...
.hub a .val{font-family:'Montserrat', sans-serif;font-size:13px;color:var(--lime);
...
.dates button small{display:block;font-weight:600;font-size:8.5px;letter-spacing:.14em;text-transform:uppercase;
  margin-bottom:4px}
.dates button b{display:block;font-family:'Montserrat', sans-serif;font-size:12px;font-weight:400}
...
.feat{position:relative;border-radius:22px;padding:24px 22px 30px;border:1px solid var(--hair);
  ...
  background:radial-gradient(ellipse at 50% 128%,rgba(24,0,173,.17),rgba(255,255,255,.02) 64%)}
...
.feat .sc{font-family:'Montserrat', sans-serif;font-size:clamp(28px,3.2vw,40px);font-weight:300;
  letter-spacing:-.03em;white-space:nowrap}
.minute{position:absolute;left:50%;bottom:-13px;transform:translateX(-50%);border-radius:999px;padding:6px 15px;
  background:var(--lime);color:#fff;font-family:'Montserrat', sans-serif;font-size:11px;white-space:nowrap;
  box-shadow:0 12px 28px -12px rgba(24,0,173,.9)}
...
.fixt .stat{text-align:center;font-family:'Montserrat', sans-serif;font-size:11px;color:var(--faint);
  line-height:1.4}
...
.fixt .tm .g{margin-left:auto;font-family:'Montserrat', sans-serif;font-size:13.5px;flex:none}
.fixt .tm.dim .n,.fixt .tm.dim .g{color:var(--faint)}
.fixt .mine{font-family:'Montserrat', sans-serif;font-size:9.5px;color:var(--lime);flex:none;
  border:1px solid rgba(24,0,173,.28);background:rgba(24,0,173,.08);border-radius:6px;padding:2px 6px}
...
.tabstrip button[aria-pressed="true"]{background:var(--lime);color:#fff;box-shadow:var(--inset)}
...
.form5 i{width:21px;height:21px;border-radius:7px;display:grid;place-items:center;font-style:normal;
  font-family:'Montserrat', sans-serif;font-size:9.5px;border:1px solid var(--hair);
  background:rgba(255,255,255,.05);color:var(--faint)}
.form5 i.w{background:rgba(24,0,173,.15);border-color:rgba(24,0,173,.36);color:var(--lime)}
...
.gbar i{position:absolute;left:0;right:0;bottom:0;border-radius:99px;
  background:linear-gradient(180deg,#1800ad,#6f9c00);transition:height .9s var(--ease)}
.gbar.rival i{background:linear-gradient(180deg,rgba(255,255,255,.42),rgba(255,255,255,.14))}
.gside .pc{font-family:'Montserrat', sans-serif;font-size:19px;font-weight:300}
...
.rung{position:relative;z-index:1;min-width:38px;text-align:center;border-radius:11px;padding:7px 10px;
  border:1px solid var(--hair);background:rgba(10,11,12,.96);font-family:'Montserrat', sans-serif;
  font-size:12px;color:var(--faint);box-shadow:var(--inset)}
.rung.you{border-color:var(--lime);background:rgba(24,0,173,.1);color:var(--lime)}
...
@media (max-width:768px){
  ...
  .app-head h1{font-size:clamp(26px,8vw,40px)!important;font-weight:700!important}
  ...
  .taskbar{left:0!important;right:0!important;bottom:0!important;transform:none!important;
    width:100%!important;max-width:none!important;border-radius:0!important;margin:0!important;
    padding:0 0 env(safe-area-inset-bottom, 0px)!important}
  .taskbar a{height:54px;padding:0}
...
.mkrow .lev{font-family:'Montserrat', sans-serif;font-size:8.5px;color:var(--faint);
...
.mkrow .px{text-align:right;font-family:'Montserrat', sans-serif;font-size:13.5px;font-variant-numeric:tabular-nums}
.mkrow .px em{display:block;font-style:normal;font-size:10px;color:var(--faint);margin-top:3px}
.pct{display:block;text-align:center;border-radius:7px;padding:8px 0;font-family:'Montserrat', sans-serif;
  font-size:12px;font-weight:500;background:rgba(24,0,173,.14);color:var(--lime);
  border:1px solid rgba(24,0,173,.3)}
...
.stp input{flex:1;min-width:0;border:0;background:transparent;color:var(--ink);outline:none;
  font-family:'Montserrat', sans-serif;font-size:14px;text-align:right;font-variant-numeric:tabular-nums}
...
.stp button:hover{color:var(--lime);border-color:rgba(24,0,173,.4)}
...
.slider .knob{position:absolute;width:15px;height:15px;border-radius:99px;background:var(--lime);
  transform:translateX(-50%);box-shadow:0 2px 10px rgba(24,0,173,.7);pointer-events:none;
...
.slider .pcts{position:absolute;top:22px;left:0;right:0;display:flex;justify-content:space-between;
  font-family:'Montserrat', sans-serif;font-size:9px;color:var(--faint)}

/* ── candlestick chart ──────────────────────────────────────── */
.candles{position:relative;height:280px}
.candles svg{width:100%;height:100%;display:block;overflow:visible}
.cx{display:flex;justify-content:space-between;font-family:'Montserrat', sans-serif;font-size:9px;
  color:var(--faint);margin-top:8px}
.ohlc{display:flex;gap:16px;flex-wrap:wrap;font-family:'Montserrat', sans-serif;font-size:10.5px;
  color:var(--faint)}
...
.stickybar .buy{background:var(--lime);color:#fff}
...
.book2 .last b{font-family:'Montserrat', sans-serif;font-size:17px;font-weight:400}
...
.b2row{position:relative;display:grid;grid-template-columns:1fr 1fr;gap:10px;padding:5px 6px;
  font-family:'Montserrat', sans-serif;font-size:11.5px;border-radius:5px;cursor:pointer}
...
.b2row.bid i{background:rgba(24,0,173,.12)}
...
.depthkey{display:flex;justify-content:space-between;font-family:'Montserrat', sans-serif;font-size:9px;
  margin-top:6px}
...
body.app .app-head h1{font-size:clamp(19px,2vw,24px);font-weight:700;
...
body.app .phead h1{font-size:clamp(19px,2vw,24px)!important;
  font-weight:700!important;margin-top:10px}
body.app .phead .lede{font-size:12.5px;margin-top:9px}
body.app h2{font-size:17px!important;font-weight:700!important}
...
body.app .taskbar a{height:54px;padding:0}
body.app .taskbar a.on{padding:0}
...
.nav-island.topbar #navBell .ic{width:16px;height:16px}
.nav-island.topbar #navBell .dot{top:3px;right:3px;width:6px;height:6px}
...
.kc-p-back:hover,.kc-p-action-btn:hover{background:rgba(255,255,255,.1);border-color:rgba(255,255,255,.15);color:#fff}
body.app main{padding-top:76px!important}
body.app .kc-home-wrap,body.app .kc-ex-wrap,body.app .kc-wallet-wrap,body.app .kc-profile-wrap,body.app .kc-trade-wrap,body.app .kc-assets-wrap,body.app .wrap{padding-top:0!important}
body.app .taskbar ~ footer{padding-bottom:104px}
@media (max-width:768px){
  .nav-island.topbar{top:max(10px,env(safe-area-inset-top));left:10px;right:10px;transform:none;width:auto;max-width:calc(100vw - 20px)!important;height:43px;min-height:43px;padding:0 14px}
  .nav-island.topbar .logo{font-size:13px}
  .nav-island.topbar .logo .ic{width:19px;height:19px}
  .nav-island.topbar #navBell{width:26px;height:26px}
  .nav-island.topbar #navBell .ic{width:15px;height:15px}
  .nav-profile-btn{width:26px;height:26px}
  body.app main{padding-top:64px!important}
  body.app .kc-home-wrap,body.app .kc-ex-wrap,body.app .kc-wallet-wrap,body.app .kc-profile-wrap,body.app .kc-trade-wrap,body.app .kc-assets-wrap,body.app .wrap{padding-top:0!important}
...
SHADOW_CSS = r"""
/* A restrained elevation scale keeps the dense app readable without neon glow. */
:root{
  --shadow-card:0 8px 22px rgba(0,0,0,.26),inset 0 1px 0 rgba(255,255,255,.025);
  --shadow-raised:0 14px 34px rgba(0,0,0,.38),inset 0 1px 0 rgba(255,255,255,.035);
  --shadow-float:0 16px 38px rgba(0,0,0,.5),0 1px 0 rgba(255,255,255,.04) inset;
  --shadow-action:0 8px 20px rgba(132,177,0,.18);
}
body.app .bezel>.core:not(.mobile-flat),
body.app .kc-group-box,
body.app .kc-ref-card,
body.app .kc-home-bal-card,
body.app .kc-assets-card,
body.app .fp-panel,
body.app .div-card{box-shadow:var(--shadow-card)!important}
body.app .nav-island.topbar,
body.app .taskbar,
body.app .kc-action-dock{box-shadow:var(--shadow-float)!important}
body.app .btn-lime,
body.app .kc-btn-buy{box-shadow:var(--shadow-action)!important}
body.app .kc-p-avatar-img{box-shadow:0 8px 24px rgba(0,0,0,.35),0 0 0 4px rgba(24,0,173,.055)!important}
body.app .kc-row:hover,
body.app .kc-item-row:hover{box-shadow:inset 3px 0 0 rgba(24,0,173,.42)}
@media(max-width:768px){
  :root{--shadow-card:0 5px 16px rgba(0,0,0,.22);--shadow-float:0 10px 28px rgba(0,0,0,.46)}
  body.app .mobile-flat{box-shadow:none!important}
}
"""
...
body.app{
  ...
  background:#05030d;color:var(--ink);font-family:Montserrat,system-ui,sans-serif;
  font-size:14px;line-height:1.45;min-height:100vh;padding-bottom:102px
}
body.app button,body.app input,body.app select,body.app textarea{font-family:Montserrat,system-ui,sans-serif}
...
body.app .taskbar .tb-fill-icon{width:26px;height:26px;display:block;fill:currentColor}
...
body.app .searchbox:focus-within,body.app .kc-search-box:focus-within,body.app .kc-home-search-box:focus-within,body.app .tf .inp:focus-within,body.app .field:focus-within{
  border-color:var(--hair)!important;outline:none!important;box-shadow:none!important
}
...
TAB_OF = {"dashboard.html": "dashboard.html", "clubs.html": "dashboard.html",
          "club-builder.html": "dashboard.html", "onboarding.html": "dashboard.html",
...
def taskbar(current=""):
    """Dock five-item bottom bar sitting flat across screen bottom."""
    if current == "asset.html":
        return ""
    tab = TAB_OF.get(current, "")
    # Filled icons (Noun Project; credits in public/ICON-CREDITS.md). They take
    # currentColor, so the active tab can recolour them without a second asset.
    line_icons = {
        "home": '<svg class="tb-fill-icon" viewBox="6 6 88 88" aria-hidden="true"><path d="m87.379 28.5-35.355-19.641c-1.2578-0.69922-2.7891-0.69922-4.0469 0l-35.355 19.641c-2.6445 1.4688-4.2852 4.2578-4.2891 7.2852v47.547c0.007813 4.6016 3.7344 8.3281 8.3359 8.3359h66.664c4.6016-0.007813 8.3281-3.7344 8.3359-8.3359v-47.547c-0.003907-3.0273-1.6445-5.8164-4.2891-7.2852zm-4.0469 54.832-66.664 0.007813v-47.555l33.332-18.52 33.332 18.52z"/><path d="m45.941 36.547c-0.76172 3.3008-2.4375 6.3242-4.8359 8.7227-2.3984 2.3984-5.4219 4.0742-8.7227 4.8398-1.8867 0
-1.21094-.00976l-8.10022,6.06738h-.808A7.06559,7.06559,0,0,0,3.58984,20.8877V35.21289a7.06559,7.06559,0,0,0,7.05762,7.05762H37.35254a7.06559,7.06559,0,0,0,7.05762-7.05762V20.8877c0-.20441-.01355-.40558-.0307-.60566A.94229.94229,0,0,0,44.41016,20.12988ZM32.22168,8.78418l4.57324,5.0459H28.6084l-2.52973-1.95416ZM20.14551,9.82031l5.19177,4.00977H14.79266Zm-9.49805,6.00977H37.35254a5.05165,5.05165,0,0,1,4.72467,3.2998H5.92285A5.05157,5.05157,0,0,1,10.64746,15.83008ZM37.35254,40.27051H10.64746a5.063,5.063,0,0,1-5.05762-5.05762v-14.083H42.41016v14.083A5.063,5.063,0,0,1,37.35254,40.27051Z"/></svg>'
    }
    out = []
...
function drawCandles(el, data){
  ...
  for(var g = 0; g <= 3; g++){
    ...
    grid.push('<text x="' + (w - padR + 7) + '" y="' + (gy + 3.5).toFixed(1)
      + '" fill="#5A605B" font-size="10" font-family="Montserrat, sans-serif">'
      + gv.toFixed(2) + '</text>');
  }
  data.forEach(function(d, i){
    var x = i * cw + cw / 2, up = d.c >= d.o;
    var col = up ? '#1800ad' : '#FF5E5E';
...
JS_SHELL = JS_CHART + r"""
if(typeof window.nextPage!=='function'){window.nextPage=function(){try{var raw=(new URLSearchParams(window.location.search)).get('next')||'';raw=raw.split('/').pop().split('?')[0].replace(/\.html$/i,'').toLowerCase();return raw?raw+'.html':null;}catch(e){return null;}};}
...
function spark(up,w,h){
  ...
  return "<svg class='spark' viewBox='0 0 "+w+" "+h+"' preserveAspectRatio='none'><polyline points='"+pts.join(' ')+
    "' fill='none' stroke='"+(up?'#1800ad':'#FF5E5E')+"' stroke-width='1.2' stroke-linejoin='round' opacity='.9'/></svg>";
}
var FT_TICKERS={
  'FSAKA':'FSAKA', 'FHLND':'FHLND', 'FKM7':'FKM7', 'FVJR':'FVJR',
  'FBEL':'FBEL', 'FPLMR':'FPLMR', 'FYAML':'FYAML', 'FMUS':'FMUS',
  'FWRTZ':'FWRTZ', 'FRODR':'FRODR', 'FFODN':'FFODN', 'FPEDR':'FPEDR',
  'FSALI':'FSALI', 'FBRN':'FBRN', 'FJACK':'FJACK', 'FARTA':'FARTA',
  'FPEP':'FPEP', 'FMARS':'FMARS', 'FAITN':'FAITN', 'FPUTL':'FPUTL', 'FKERR':'FKERR', 'FRUSS':'FRUSS', 'FLJMS':'FLJMS', 'FWILM':'FWILM', 'FEARP':'FEARP', 'FWIEG':'FWIEG', '$CR7':'FCR7', '$Ronaldo':'FCR7', '$Messi':'FLM10',
  ...
  'Pep':'FPEP', 'Maresca':'FMARS', 'Bonmati':'FAITN', 'Aitana':'FAITN', 'Putellas':'FPUTL', 'Alexia':'FPUTL', 'Kerr':'FKERR', 'Russo':'FRUSS', 'James':'FLJMS', 'Williamson':'FWILM', 'Earps':'FEARP', 'Wiegman':'FWIEG', 'CR7':'FCR7', 'Ronaldo':'FCR7', 'Messi':'FLM10'
};
...
/* `lg` is the league an asset plays in; the Exchange filters on it rather
   ...
   under the Premier League. `w` marks the women's game. */
var ASSETS=[
 {t:'FSAKA',n:'Bukayo Saka',p:6.3052,v:126104000,d:6.4,c:false,cap:'63.1M',h:6.69764,low:6.122061,q:'FTR',vol:'1.6M',tag:'10x',pos:'FWD',club:'Arsenal',lg:'Premier League'},
 {t:'FHLND',n:'Erling Haaland',p:12.6104,v:252208000,d:-1.8,c:false,cap:'126.1M',h:13.069602,low:12.380799,q:'FTR',vol:'3.3M',tag:'10x',pos:'FWD',club:'Manchester City',lg:'Premier League'},
 {t:'FKM7',n:'Kylian Mbappé',p:11.464,v:229280000,d:4.8,c:false,cap:'114.6M',h:11.829096,low:11.128112,q:'FTR',vol:'3.6M',tag:'10x',pos:'FWD',club:'Real Madrid',lg:'La Liga'},
 {t:'FVJR',n:'Vinícius Júnior',p:8.0248,v:160496000,d:0.7,c:false,cap:'80.2M',h:8.266434,low:7.884906,q:'FTR',vol:'1.9M',tag:'10x',pos:'FWD',club:'Real Madrid',lg:'La Liga'},
 {t:'FBEL',n:'Jude Bellingham',p:9.1712,v:183424000,d:3.3,c:false,cap:'91.7M',h:9.498187,low:8.906496,q:'FTR',vol:'2.2M',tag:'10x',pos:'MID',club:'Real Madrid',lg:'La Liga'},
 {t:'FPLMR',n:'Cole Palmer',p:5.732,v:114640000,d:8.2,c:false,cap:'57.3M',h:5.916553,low:5.330326,q:'FTR',vol:'1.8M',tag:'10x',pos:'MID',club:'Chelsea',lg:'Premier League'},
 {t:'FYAML',n:'Lamine Yamal',p:12.6104,v:252208000,d:9.4,c:false,cap:'126.1M',h:12.953281,low:11.7151,q:'FTR',vol:'4.2M',tag:'10x',pos:'FWD',club:'Barcelona',lg:'La Liga'},
 {t:'FMUS',n:'Jamal Musiala',p:5.732,v:114640000,d:5.1,c:false,cap:'57.3M',h:5.952934,low:5.425148,q:'FTR',vol:'1.4M',tag:'10x',pos:'MID',club:'Bayern Munich',lg:'Bundesliga'},
 {t:'FWRTZ',n:'Florian Wirtz',p:5.732,v:114640000,d:3.8,c:false,cap:'57.3M',h:5.937632,low:5.526368,q:'FTR',vol:'1.4M',tag:'10x',pos:'MID',club:'Liverpool',lg:'Premier League'},
 {t:'FRODR',n:'Rodri',p:2.866,v:57320000,d:1.4,c:false,cap:'28.7M',h:2.922104,low:2.809896,q:'FTR',vol:'640.5K',tag:'10x',pos:'MID',club:'Barcelona',lg:'La Liga'},
 {t:'FFODN',n:'Phil Foden',p:4.5856,v:91712000,d:-1.2,c:false,cap:'45.9M',h:4.746647,low:4.509315,q:'FTR',vol:'1.0M',tag:'10x',pos:'FWD',club:'Manchester City',lg:'Premier League'},
 {t:'FPEDR',n:'Pedri González',p:8.598,v:171960000,d:-0.9,c:false,cap:'86.0M',h:8.880073,low:8.37861,q:'FTR',vol:'1.9M',tag:'10x',pos:'MID',club:'Barcelona',lg:'La Liga'},
 {t:'FSALI',n:'William Saliba',p:5.732,v:114640000,d:2.7,c:false,cap:'57.3M',h:5.935503,low:5.49458,q:'FTR',vol:'1.2M',tag:'10x',pos:'DEF',club:'Arsenal',lg:'Premier League'},
 {t:'FBRN',n:'Bruno Fernandes',p:2.0062,v:40124000,d:2.1,c:false,cap:'20.1M',h:2.109664,low:1.943112,q:'FTR',vol:'494.6K',tag:'10x',pos:'MID',club:'Manchester United',lg:'Premier League'},
 {t:'FJACK',n:'Nicolas Jackson',p:2.2928,v:45856000,d:11.2,c:false,cap:'22.9M',h:2.393158,low:2.038044,q:'FTR',vol:'833.7K',tag:'10x',pos:'FWD',club:'Aston Villa',lg:'Premier League'},
 {t:'FAITN',n:'Aitana Bonmatí',p:0.091712,v:1834240,d:1.9,c:false,cap:'917.1K',h:0.094373,low:0.088437,q:'FTR',vol:'13.9K',tag:'10x',pos:'MID',club:'Barcelona',lg:'Liga F',w:1},
 {t:'FPUTL',n:'Alexia Putellas',p:0.068784,v:1375680,d:5.6,c:false,cap:'687.8K',h:0.070296,low:0.064438,q:'FTR',vol:'14.9K',tag:'10x',pos:'MID',club:'London City Lionesses',lg:'WSL',w:1},
 {t:'FRUSS',n:'Alessia Russo',p:0.074516,v:1490320,d:2.4,c:false,cap:'745.2K',h:0.07624,low:0.072026,q:'FTR',vol:'11.7K',tag:'10x',pos:'FWD',club:'Arsenal',lg:'WSL',w:1},
 {t:'FLJMS',n:'Lauren James',p:0.040124,v:802480,d:4.2,c:false,cap:'401.2K',h:0.041473,low:0.038213,q:'FTR',vol:'6.6K',tag:'10x',pos:'FWD',club:'Chelsea',lg:'WSL',w:1},
 {t:'FKERR',n:'Sam Kerr',p:0.02866,v:573200,d:3.1,c:false,cap:'286.6K',h:0.029395,low:0.02765,q:'FTR',vol:'4.8K',tag:'10x',pos:'FWD',club:'Gotham FC',lg:'NWSL',w:1},
 {t:'FWILM',n:'Leah Williamson',p:0.04299,v:859800,d:0.8,c:false,cap:'429.9K',h:0.043928,low:0.042208,q:'FTR',vol:'5.3K',tag:'10x',pos:'DEF',club:'Arsenal',lg:'WSL',w:1},
 {t:'FEARP',n:'Mary Earps',p:0.006878,v:137568,d:-0.6,c:false,cap:'68.8K',h:0.007048,low:0.006737,q:'FTR',vol:'877',tag:'10x',pos:'GK',club:'London City Lionesses',lg:'WSL',w:1},
 {t:'FARTA',n:'Mikel Arteta',p:1.1025,v:22050000,d:4.9,c:true,cap:'11.0M',h:1.175,low:1.05,q:'FTR',vol:'305.0K',tag:'COACH',pos:'MGR',club:'Arsenal',lg:'Premier League'},
 {t:'FPEP',n:'Pep Guardiola',p:1.48,v:29600000,d:-0.4,c:true,cap:'14.8M',h:1.54,low:1.445,q:'FTR',vol:'390.0K',tag:'COACH',pos:'MGR',club:'Manchester City',lg:'Premier League'},
 {t:'FMARS',n:'Enzo Maresca',p:0.915,v:18300000,d:1.2,c:true,cap:'9.2M',h:0.96,low:0.89,q:'FTR',vol:'225.0K',tag:'COACH',pos:'MGR',club:'Chelsea',lg:'Premier League'},
 {t:'FWIEG',n:'Sarina Wiegman',p:1.34,v:26800000,d:1.5,c:true,cap:'13.4M',h:1.37,low:1.305,q:'FTR',vol:'200.0K',tag:'COACH',pos:'MGR',club:'England',lg:'International',w:1}
];
/* Players Fantrade has cleared for listing that nobody has launched yet.
   The first manager to claim one pays for the shares they take at the
   reference price below, and the player goes live on the Exchange for
   everyone else to buy. The prices are demo figures, not valuations. */
var ELIGIBLE=[
 {t:'FCR7',n:'Cristiano Ronaldo',aka:'CR7',p:0.5732,v:11464000,pos:'FWD',club:'Al Nassr',lg:'Saudi Pro League'},
 {t:'FLM10',n:'Lionel Messi',p:0.8598,v:17196000,pos:'FWD',club:'Inter Miami',lg:'MLS'},
 {t:'FKANE',n:'Harry Kane',p:3.4392,v:68784000,pos:'FWD',club:'Bayern Munich',lg:'Bundesliga'},
 {t:'FRICE',n:'Declan Rice',p:6.8784,v:137568000,pos:'MID',club:'Arsenal',lg:'Premier League'},
 {t:'FVVD',n:'Virgil van Dijk',aka:'VVD',p:1.60496,v:32099200,pos:'DEF',club:'Liverpool',lg:'Premier League'},
 {t:'FOSIM',n:'Victor Osimhen',p:4.299,v:85980000,pos:'FWD',club:'Galatasaray',lg:'Süper Lig'},
 {t:'FSALH',n:'Mohamed Salah',aka:'Mo Salah',p:1.26104,v:25220800,pos:'FWD',club:'Trabzonspor',lg:'Süper Lig'},
 {t:'FLOOK',n:'Ademola Lookman',p:2.2928,v:45856000,pos:'FWD',club:'Atlético Madrid',lg:'La Liga'},
 {t:'FCHUK',n:'Samuel Chukwueze',p:1.1464,v:22928000,pos:'FWD',club:'AC Milan',lg:'Serie A'},
 {t:'FSHAW',n:'Khadija Shaw',aka:'Bunny Shaw',p:0.041557,v:831140,pos:'FWD',club:'Manchester City',lg:'WSL',w:1},
 {t:'FPAJR',n:'Ewa Pajor',p:0.022928,v:458560,pos:'FWD',club:'Barcelona',lg:'Liga F',w:1},
 {t:'FRODM',n:'Trinity Rodman',p:0.020062,v:401240,pos:'FWD',club:'Washington Spirit',lg:'NWSL',w:1},
 {t:'FKELY',n:'Chloe Kelly',p:0.051588,v:1031760,pos:'FWD',club:'Arsenal',lg:'WSL',w:1},
 {t:'FAJBD',n:'Rasheedat Ajibade',p:0.024361,v:487220,pos:'FWD',club:'Paris Saint-Germain',lg:'Première Ligue',w:1},
 {t:'FALOZ',n:'Michelle Alozie',p:0.004586,v:91712,pos:'DEF',club:'Chicago Stars',lg:'NWSL',w:1},
 {t:'FLENR',n:'Luis Enrique',p:1.3,v:26000000,pos:'MGR',club:'Paris Saint-Germain',lg:'Ligue 1'},
 {t:'FSIME',n:'Diego Simeone',p:1,v:20000000,pos:'MGR',club:'Atlético Madrid',lg:'La Liga'},
 {t:'FFLCK',n:'Hansi Flick',p:1.2,v:24000000,pos:'MGR',club:'Barcelona',lg:'La Liga'},
 {t:'FANCE',n:'Carlo Ancelotti',p:1.1,v:22000000,pos:'MGR',club:'Brazil',lg:'International'},
 {t:'FHAYS',n:'Emma Hayes',p:0.45,v:9000000,pos:'MGR',club:'United States',lg:'International',w:1}
];
/* The $FTR economy. $FTR is capped at 10,000,000 and priced by the market
   (start: $2). Every player has 10,000,000 shares; a share is worth their
   dollar valuation (v) / 10,000,000, and costs that in $FTR at the current
   $FTR price, so when $FTR moves, every $FTR price moves with it. The live
   price is fetched by fantrade-supabase.js and cached on this device. */
var FTR_USD=2, FTR_MAX_SUPPLY=10000000;
function ftReprice(){
  [ASSETS, typeof ELIGIBLE !== 'undefined' ? ELIGIBLE : []].forEach(function(list){ list.forEach(function(a){
    if(!(a.v > 0)) return;
    var np = +(a.v / 1e7 / FTR_USD).toFixed(6), old = Number(a.p) || np;
    if(a.h) a.h = +(a.h * np / old).toFixed(6);
    if(a.low) a.low = +(a.low * np / old).toFixed(6);
    a.p = np; a.cap = (np * 10 >= 1 ? (np * 10).toFixed(1) + 'M' : Math.round(np * 1e7).toLocaleString('en-US'));
  }); });
}
try { var _ftM = JSON.parse(localStorage.getItem('ft_market_v1') || 'null'); if(_ftM && Number(_ftM.ftr_usd) > 0) FTR_USD = Number(_ftM.ftr_usd); } catch(e){}
ftReprice();
window.addEventListener('fantrade:market', function(ev){
  if(ev.detail && Number(ev.detail.ftr_usd) > 0 && Number(ev.detail.ftr_usd) !== FTR_USD){ FTR_USD = Number(ev.detail.ftr_usd); ftReprice(); }
});
/* Share prices run from tens of $FTR down to fractions of a cent, so they get
   as many decimals as they need: 12.61, 0.0917, 0.004586. */
function pxFix(n){ n = Number(n) || 0; var a = Math.abs(n); return n.toFixed(a >= 1 || a === 0 ? 2 : a >= 0.01 ? 4 : 6); }
/* Live quotes carry one more decimal so small moves show: 6.312, 12.604. */
function pxLive(n){ n = Number(n) || 0; var a = Math.abs(n), d = a >= 100 ? 2 : a >= 1 ? 3 : a >= 0.01 ? 4 : 6;
  return n.toLocaleString('en-US', { minimumFractionDigits: d, maximumFractionDigits: d }); }
function pxFmt(n){ n = Number(n) || 0; var a = Math.abs(n), d = a >= 1 || a === 0 ? 2 : a >= 0.01 ? 4 : 6;
  return n.toLocaleString('en-US', { minimumFractionDigits: d, maximumFractionDigits: d }); }
function usdFmt(ftr){ var d = (Number(ftr) || 0) * FTR_USD, a = Math.abs(d);
  if(a >= 1e6) return '$' + (d / 1e6).toFixed(2).replace(/\.?0+$/, '') + 'M';
  return '$' + pxFmt(d); }
/* Filled in at build time from assets/players/attribution.json: a picture
   is only used when its source and licence are on record. */
var PLAYER_IMAGES=__VERIFIED_PHOTOS__;
/* Initials on a tint picked from the name, as an SVG the <img> can load, so
   every place that shows a photo can show this instead without changes. */
function monogram(name){
  var words = String(name || '?').replace(/[^A-Za-zÀ-ɏ ]/g, ' ').trim().split(/\s+/);
  var ini = ((words[0] || '?').charAt(0) + (words.length > 1 ? words[words.length - 1].charAt(0) : '')).toUpperCase();
  var hues = ['#1800ad', '#3a1fbf', '#0b5fae', '#5a2d9c', '#14418f', '#2c2396'];
  var h = 0; for (var i = 0; i < ini.length + String(name).length; i++) h = (h * 31 + String(name).charCodeAt(i % String(name).length)) >>> 0;
  var svg = "<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'>"
    + "<rect width='100' height='100' fill='" + hues[h % hues.length] + "'/>"
    + "<text x='50' y='50' dy='.36em' text-anchor='middle' font-family='Montserrat,Arial,sans-serif' "
    + "font-weight='700' font-size='38' fill='#ffffff'>" + ini + "</text></svg>";
  // encodeURIComponent leaves ' alone, and photos are written into
  // single-quoted src attributes, so it has to be encoded by hand.
  return 'data:image/svg+xml;charset=utf-8,' + encodeURIComponent(svg).replace(/'/g, '%27');
}
function playerPhoto(symbol,name,className){
  var raw = String(symbol||'').replace(/^\$/, '');
  var sLower = raw.toLowerCase();
  var key = sLower;
  if(sLower.startsWith('f') && sLower.length > 3){
    var map = { fsaka:'saka', fhlnd:'haaland', fkm7:'mbappe', fvjr:'vinicius', fbel:'bellingham', fplmr:'palmer', fyaml:'yamal', fmus:'musiala', fwrtz:'wirtz', frodr:'rodri', ffodn:'foden', fpedr:'pedri', fsali:'saliba', fbrn:'bruno', fjack:'jackson', farta:'arteta', fpep:'pep', fmars:'maresca', faitn:'bonmati', fputl:'putellas', fkerr:'kerr', fruss:'russo', fljms:'james', fwilm:'williamson', fearp:'earps', fwieg:'wiegman', fvvd:'vandijk', frice:'rice', fkane:'kane', fsalh:'salah' };
...
function applyProfiles(rows){
  (rows || []).forEach(function(p){
    ...
    if(a){ if(p.club) a.club = p.club; if(p.lg) a.lg = p.lg; if(p.gender === 'W') a.w = 1; if(Number(p.v) > 0) a.v = Number(p.v); }
    ...
    if(e){ if(p.club) e.club = p.club; if(p.lg) e.lg = p.lg; if(p.about) e.about = p.about; if(p.country) e.country = p.country;
           if(p.known_as) e.aka = p.known_as; if(p.gender === 'W') e.w = 1; if(Number(p.v) > 0) e.v = Number(p.v); }
  });
  if(typeof ftReprice === 'function') ftReprice();
}
...
function liveTicks(sel){
  ...
  setInterval(function(){
    ...
    if(px){ px.textContent=a.p.toFixed(2); px.style.color=move>=0?'#1800ad':'#FF5E5E';
...
var FT = (function(){
  ...
  var defaultState = {
    ...
    notifications: [
      ...
      { id:"n-2", kind:"order", icon:"candle", title:"Buy order filled", read:false, day:"Today",
        msg:"10,000 FSAKA filled at an average of 6.31 $FTR ($12.61). Average cost basis moved to 4.11.",
        time:"08:02 · 3h ago", amt:"-63,304 $FTR", tone:"down" },
      ...
      { id:"n-4", kind:"settle", icon:"coin", title:"Dividend distributed", read:true, day:"Yesterday",
        msg:"FSAKA clean sheet and man-of-the-match distribution paid to all holders on the ledger.",
        time:"Yesterday · 22:10", amt:"+21 $FTR", tone:"up" },
      { id:"n-5", kind:"club", icon:"bolt", title:"Coach synergy unlocked", read:true, day:"Yesterday",
        msg:"FARTA now matches your 4-3-3 shape. Club multiplier rose from 10.0% to 15.0%.",
        time:"Yesterday · 19:22", amt:"", tone:"" },
      { id:"n-6", kind:"order", icon:"swap", title:"Limit order expired", read:true, day:"Yesterday",
        msg:"Your limit buy for 2,000 FPEDR at 8.40 $FTR expired unfilled. Nothing was charged.",
    ...
    wallet: {
      ...
      locked: 0,
      seasonEarned: 19640,
      usdRate: 0.5      // $1 buys 0.5 $FTR at $2 a $FTR
    },
    holdings: {
      'FSAKA': { n: 'Bukayo Saka', shares: 10000, avg: 4.1075, p: 6.3052, c: false, inClub: 'RW' },
      'FBRN': { n: 'Bruno Fernandes', shares: 5000, avg: 1.9179, p: 2.0062, c: false, inClub: 'CAM' },
      'FHLND': { n: 'Erling Haaland', shares: 3000, avg: 12.0981, p: 12.6104, c: false, inClub: 'ST' },
      'FARTA': { n: 'Mikel Arteta', shares: 1000, avg: 1.025, p: 1.1025, c: true, inClub: 'COACH' }
    },
    club: {
    ...
    transactions: [
      { type: "BUY", asset: "FSAKA", shares: 10000, price: 6.3052, total: 63304, time: "Today, 14:22" },
      { type: "STAKE", asset: "FanPlay MD 07", shares: 1, price: 125, total: 125, time: "Yesterday, 19:10" },
      { type: "PAYOUT", asset: "Matchday 06 Settle", shares: 1, price: 310, total: 310, time: "Sep 08, 02:15" },
      { type: "CONVERT", asset: "USD Deposit ($1,000)", shares: 1000, price: 0.5, total: 497.5, time: "Sep 07, 11:45" }
    ]
  };
  ...
  function link(st){
    ...
    // Demo accounts start with 1,000,000 $FTR. A balance saved before that
    // is lifted once; after that it is whatever the manager makes of it.
    if(!st.demoGrant1M){
      if(st.wallet && Number(st.wallet.balance) < 1000000) st.wallet.balance = 1000000;
      st.demoGrant1M = true;
    }
    st.listedAssets = st.listedAssets || [];
    if(st.wallet){ delete st.wallet.gbpRate; st.wallet.usdRate = 1 / FTR_USD; }
    if(!st.econV2){
      Object.keys(st.holdings || {}).forEach(function(t){
        var h = st.holdings[t], a = ASSETS.filter(function(x){ return x.t === t; })[0];
        var np = a ? a.p : (Number(h.p) || 0) * 0.05;
        if(Number(h.p) > 0){ h.avg = +(Number(h.avg || h.p) * np / Number(h.p)).toFixed(6); } else h.avg = np;
        h.p = np;
      });
      st.listedAssets.forEach(function(a){ if(!a.v && Number(a.p) > 0){ a.v = Math.round(Number(a.p) * 0.10 * 1e7); } });
      if(st.wallet){ delete st.wallet.gbpRate; }
      st.econV2 = true;
    }
    delete st.clubs;
    delete st.activeClub;
    st.fanplay = st.fanplay || {};
    st.fanplay.activeEntries = st.fanplay.activeEntries || [];
    st.fanplay.history = st.fanplay.history || [];
    // The two seeded demo entries (e-1, e-2) were never placed by anyone.
    var seeded = st.fanplay.activeEntries.filter(function(e){ return e.id === 'e-1' || e.id === 'e-2'; });
    if(seeded.length){
      st.fanplay.activeEntries = st.fanplay.activeEntries.filter(function(e){ return seeded.indexOf(e) < 0; });
      if(st.wallet) st.wallet.locked = Math.max(0, (st.wallet.locked || 0) - seeded.reduce(function(t, e){ return t + (Number(e.stake) || 0); }, 0));
    }
    return st;
  }
  function load(){
    try {
      ...
      if(s) return link(Object.assign({}, defaultState, JSON.parse(s)));
    } catch(e){}
    return link(JSON.parse(JSON.stringify(defaultState)));
  }
  function save(s){
    try {
      ...
      if(s.club){
        out.club = s.club;
        out.clubs = [s.club];
        out.activeClub = s.club.id || 'c-1';
      }
      localStorage.setItem(STORAGE_KEY, JSON.stringify(out));
    } catch(e){}
  }
  var state = load();
  /* Players launched from this device join the market on every page. */
  (state.listedAssets || []).forEach(function(a){
    if(typeof ASSETS !== 'undefined' && !ASSETS.some(function(x){ return x.t === a.t; })) ASSETS.push(a);
  });


  /* ── The database, kept in step with this state ──────────────────────
     The pages never wait on the network. Every action applies here first,
     then mirrors to Supabase; if the database refuses, we take its answer
     as the truth and re-sync. */
  function cloud(){ return window.FTDB && window.FTDB.signedIn && window.FTDB.signedIn() ? window.FTDB : null; }
  function shortTime(iso){
    var d = new Date(iso), now = new Date(), mins = Math.round((now - d) / 60000);
    if(mins < 1) return 'Just now';
    if(mins < 60) return mins + 'm ago';
    if(d.toDateString() === now.toDateString()) return 'Today, ' + d.toTimeString().slice(0,5);
  ...
  function applySnapshot(snap){
    ...
    if(snap.profile){
      state.user.name = snap.profile.display_name || state.user.name;
    ...
    if(snap.wallet){
      state.wallet.balance = Number(snap.wallet.balance) || 0;
      state.wallet.locked = Number(snap.wallet.locked) || 0;
      state.wallet.seasonEarned = Number(snap.wallet.season_earned) || 0;
      state.wallet.usdRate = Number(snap.wallet.usd_rate) || (1 / FTR_USD);
    }
    if(snap.market && Number(snap.market.ftr_usd) > 0 && Number(snap.market.ftr_usd) !== FTR_USD){
      FTR_USD = Number(snap.market.ftr_usd); ftReprice();
      try { localStorage.setItem('ft_market_v1', JSON.stringify(snap.market)); } catch(e){}
    }
    if(snap.holdings){
      var held = {};
      snap.holdings.forEach(function(h){
        var coach = h.kind === 'COACH';
        // Keyed by F-ticker, the name every page uses for a share.
        held[h.ticker || ftSym(h.asset_id)] = { n: h.name, shares: Number(h.shares), avg: Number(h.avg_cost),
                             p: Number(h.price), c: coach,
                             // The slot travels with the account, so the side
                             // you set on one device is the side on the next.
                             inClub: h.slot || (coach ? 'COACH' : 'SUB') };
      });
      state.holdings = held;
    }
    if(snap.clubs && snap.clubs.length){
      ...
      state.club = (function(c){
        ...
        return { id: c.id, name: c.name, stadium: c.stadium,
                 ...
                 coach: c.coach || prev.coach || 'FARTA',
                 division: c.division, fp: Number(c.season_fp) || 0,
                 boost: Number(c.boost) || 0,
    ...
    if(snap.entries){
      // Entries made on this device carry a local id until the account has
      // confirmed them. Replacing the list wholesale threw those away — which
      // is how an entry you had just placed vanished from Active whenever the
      // save to the account failed. They are kept until the account answers.
      var unconfirmed = (state.fanplay.activeEntries || []).filter(function(e){ return !serverId(e.id); });
      // Two kinds of entry read this list — a $FTR stake on a Dream Club and
      // shares locked against a market — so a row carries both vocabularies.
      state.fanplay.activeEntries = unconfirmed.concat(snap.entries.map(function(e){
        var held = state.holdings[e.asset_id] || {};
        return { id: e.id, mode: e.mode, clubId: e.club_id || '', target: e.target,
                 tier: e.tier || '', mult: Number(e.multiplier) || 1,
                 stake: Number(e.stake) || 0,
                 stakedShares: Number(e.staked_shares) || 0,
                 asset: e.asset_id ? { symbol: e.asset_id, name: held.n || e.asset_id } : null,
                 match: e.match || null, market: e.market || null,
                 selections: e.selections || [],
                 totalFP: Number(e.scored_fp) || 0,
                 projectedFP: Number(e.projected_fp) || 0,
    ...
    if(snap.transactions){
      state.transactions = snap.transactions.map(function(t){
        return { type: t.type, asset: t.ticker || t.asset_id || t.label, shares: Number(t.shares) || 1,
                 price: Number(t.price) || 0, total: Number(t.total) || 0, time: shortTime(t.created_at) };
      });
    }
    state.auth.signedIn = true;
    if(window.FTDB && FTDB.session() && FTDB.session().user) state.auth.email = FTDB.session().user.email;
    save(state);
    return true;
  }
  function announce(){
  ...
  /* Clubs made while offline carry a local id like 'c-1'; only a uuid means
     the database has seen this one. */
  function serverId(id){ return /^[0-9a-f]{8}-[0-9a-f]{4}-/i.test(String(id || '')) ? id : null; }
  function clubArgs(c){
    return { p_name: c.name, p_stadium: c.stadium, p_formation: c.formation,
             p_colors: c.colors, p_color_name: c.colorName, p_coach: c.coach };
  }
  var pending = 0;
  /* The account could not be reached, or is missing a function it needs:
     nothing was decided, so nothing local should be undone. */
  function isOutage(error){
    return /schema cache|could not find the function|reach the server|failed to fetch|networkerror|timed? ?out|jwt|PGRST20/i
      .test(String((error && (error.code || '')) + ' ' + ((error && error.message) || error)));
  }
  function push(fn, args, onResult, onRefused){
    var db = cloud(); if(!db) return;
    pending += 1;
    db.call(fn, args).then(function(res){
      if(onResult) onResult(res);
      return db.snapshot();
    }).then(function(snap){
      if(applySnapshot(snap)) announce();
    }).catch(function(error){
      if(isOutage(error)){
        if(typeof showToast === 'function')
          showToast('Saved on this device. Your account could not be reached, so it has not synced yet.', 'info');
        return;
      }
      // The account said no. Undo what this device did, say why, then take
      // the account's figures as the truth.
      if(onRefused) onRefused(error);
      if(typeof showToast === 'function') showToast(String(error.message || 'That did not go through').replace(/[.!?]?\s*$/, '.') + ' Your balance has been re-checked.', 'error');
      return db.snapshot().then(function(snap){ if(applySnapshot(snap)) announce(); });
    }).finally(function(){ pending -= 1; });
  }

  return {
    getState: function(){ return state; },
    /* Pull everything this account holds and adopt it as the truth. */
    syncCloud: function(){
      var db = window.FTDB; if(!db) return Promise.resolve(false);
      return db.snapshot().then(function(snap){
        if(!applySnapshot(snap)) return false;
        announce(); return true;
      }).catch(function(){ return false; });
    },
    cloudSignedIn: function(){ return !!cloud(); },
    ...
    completeOnboarding: function(data){
      ...
      push('ft_update_profile', { p_display_name: state.user.name,
        p_handle: String(state.user.handle || '').replace(/^@/, ''),
        p_region: state.user.region, p_home_league: state.user.league, p_onboarded: true });
      save(state); FT.syncUI();
      window.dispatchEvent(new CustomEvent('fantrade:statechange', { detail: state }));
    },
    ...
    /* What a claim costs: the value of the shares taken, at the player's
       reference price. Level 1 is 5% of the 10,000,000 (500,000 shares),
       Level 2 is 10% (1,000,000). 2% of what is paid is burned. */
    claimQuote: function(asset, level){
      var shares = Number(level) === 2 ? 1000000 : 500000;
      var price = Number(asset && asset.p) || 0;
      var cost = Math.round(shares * price);
      var bal = Number(state.wallet.balance) || 0;
      return { level: shares === 1000000 ? 2 : 1, shares: shares, percent: shares / 100000, price: price,
               cost: cost, burn: Math.round(cost * 0.02), usd: cost * FTR_USD,
               balance: bal, short: Math.max(0, cost - bal) };
    },
    isLaunched: function(ticker){
      return ASSETS.some(function(a){ return a.t === ticker; });
    },
    /* Claim a cleared player and put them on the market. `settled` means the
       database has already taken the payment, so only the local copy moves. */
    launchPlayer: function(src, level, vestingYears, opts){
      var e = typeof src === 'object' && src ? src : ELIGIBLE.filter(function(x){ return x.t === src; })[0];
      if(!e || !e.t) throw new Error('That player has not been cleared for listing.');
      if(FT.isLaunched(e.t)) throw new Error(e.n + ' has already been claimed. You can buy their shares on the Exchange.');
      var settled = !!(opts && opts.settled);
      vestingYears = [1, 2, 3].indexOf(Number(vestingYears)) > -1 ? Number(vestingYears) : 1;
      var q = FT.claimQuote(e, level);
      if(!settled && q.short > 0){
        throw new Error('You need ' + q.short.toLocaleString('en-US') + ' more $FTR to claim ' + q.percent + '% of ' + e.n + '.');
      }
      var coach = e.pos === 'MGR';
      var asset = { t: e.t, n: e.n, p: e.p, v: e.v, d: 0, c: coach, cap: (e.p * 10).toFixed(1) + 'M', h: e.p, low: e.p,
                    q: 'FTR', vol: '0', tag: coach ? 'COACH' : '10x', pos: e.pos, club: e.club, lg: e.lg,
                    launched: Date.now(), launchedBy: state.user.handle || '' };
      if(e.w) asset.w = 1;
      if(!settled) state.wallet.balance -= q.cost;
      state.listedAssets = state.listedAssets || [];
      state.listedAssets.push(asset);
      ASSETS.push(asset);
      state.claimedListings = state.claimedListings || [];
      state.claimedListings.push('lst-' + e.t);
      var now = Date.now(), daily = Math.round(q.shares * 0.01), fp = Math.round(q.shares / 100);
      state.listerClaims = state.listerClaims || [];
      state.listerClaims.push({
        id: 'lc-' + e.t, listingId: 'lst-' + e.t, assetId: e.t, ticker: e.t, name: e.n,
        level: q.level, shares: q.shares, percent: q.percent, feePaid: q.cost, feeBurned: q.burn,
        vestingYears: vestingYears, claimedAt: now, vestingUntil: now + vestingYears * 365 * 86400000,
        dailyLimit: daily, tradedToday: 0, lastTradedDate: new Date().toISOString().slice(0, 10),
        feeSharePercent: 30, feeShareEarned: 0, listerFp: fp
      });
      state.holdings[e.t] = { n: e.n, shares: q.shares, avg: e.p, p: e.p, c: coach,
                              inClub: coach ? 'COACH' : 'SUB', isLister: true, listerDailyLimit: daily };
      state.club.fp = (state.club.fp || 0) + fp;
      state.transactions.unshift({ type: 'LAUNCH', asset: e.t, shares: q.shares, price: e.p, total: q.cost,
        label: 'Claimed ' + q.percent + '% of ' + e.n + ' (' + vestingYears + 'y vesting)', time: 'Just now' });
      if(Array.isArray(state.notifications)){
        state.notifications.unshift({ id: 'n-launch-' + e.t, kind: 'order', icon: 'star', read: false, day: 'Today',
          title: e.n + ' is live on the Exchange',
          msg: 'You claimed ' + q.shares.toLocaleString('en-US') + ' ' + e.t + ' shares, vesting over ' + vestingYears + (vestingYears === 1 ? ' year' : ' years') + '. Other managers can now buy the rest.',
          time: 'Just now', amt: '-' + q.cost.toLocaleString('en-US') + ' $FTR', tone: 'down' });
      }
      save(state); FT.syncUI();
      window.dispatchEvent(new CustomEvent('fantrade:statechange', { detail: state }));
      return { symbol: e.t, name: e.n, shares: q.shares, cost: q.cost, burned: q.burn,
               dailyLimit: daily, vestingYears: vestingYears, listerFp: fp };
    },
    claimListing: function(opts){
      ...
      var fee = Math.max(1, Math.round(shares * price * 0.002));
    ...
    clearNotifications: function(kind){
      var scope = kind || 'all';
      state.notifications = (state.notifications || []).filter(function(n){
        return scope !== 'all' && n.kind !== scope;
      });
      save(state); FT.syncUI();
      window.dispatchEvent(new CustomEvent('fantrade:statechange', { detail: state }));
    },
    executeTrade: function(side, assetSymbol, assetName, shares, price, isCoach){
      ...
      if(state.listerClaims && state.listerClaims.length){
      ...
      push(side === 'buy' ? 'ft_buy_shares' : 'ft_sell_shares', { p_asset: assetSymbol, p_shares: shares });
      save(state);
      FT.syncUI();
      window.dispatchEvent(new CustomEvent('fantrade:statechange', { detail: state }));
    ...
    /* FanPlay's other entry: shares locked against a market on one fixture.
       The page keeps the detail in its own state; this is the part that moves
       something, so it is the part the account has to know about. */
    stakeShares: function(o){
      push('ft_enter_fanplay_shares', { p_asset: o.asset, p_shares: o.shares,
        p_match: o.match || null, p_market: o.market || null,
        p_selections: o.selections || null, p_action_key: o.key || null },
        null,
        function(){
          if(!o.localId) return;
          state.fanplay.activeEntries = (state.fanplay.activeEntries || [])
            .filter(function(e){ return e.id !== o.localId; });
          save(state);
          window.dispatchEvent(new CustomEvent('fantrade:statechange', { detail: state }));
        });
    },
    /* Pulling out before the whistle: the shares come straight back. */
    cancelEntry: function(id){
      if(serverId(id)) push('ft_cancel_fanplay', { p_entry: id });
    },
    /* Where a holding lines up. 'SUB' benches it. */
    setSlot: function(symbol, slot){
      var h = state.holdings[symbol];
      if(!h) throw new Error("You do not hold any " + symbol + ".");
      h.inClub = slot;
      push('ft_set_slot', { p_asset: symbol, p_slot: slot });
      save(state);
      FT.syncUI();
      window.dispatchEvent(new CustomEvent('fantrade:statechange', { detail: state }));
      return h;
    },
    /* The board, ranked across every manager. Resolves to null when the
       account cannot be reached, and the page keeps its sample field. */
    leaderboard: function(limit){
      var db = window.FTDB; if(!db || !db.board) return Promise.resolve(null);
      return db.board(limit || 50).then(function(board){
    ...
    activateFanPlayEntry: function(entryData){
      ...
      // one club can hold one live entry, so a diversified manager stakes each separately
      if(entryData.clubId){
        var dup = (state.fanplay.activeEntries || []).filter(function(e){
          return e.clubId === entryData.clubId;
        })[0];
        if(dup){
          state.wallet.balance += stake;
          state.wallet.locked -= stake;
          throw new Error(entryData.target + " already has a live entry this round.");
        }
      }
      // The pages call these 'club' and 'solo'; the account calls them what
      // the manager sees, so one vocabulary reaches the database.
      var mode = /club/i.test(entryData.mode) ? 'Dream Club' : 'Individual';
      var newEntry = {
        id: "e-" + Date.now(),
        mode: mode,
      ...
      push('ft_enter_fanplay', { p_mode: mode, p_target: entryData.target,
        p_tier: entryData.tier, p_multiplier: entryData.mult, p_stake: stake,
        p_projected_fp: entryData.projectedFP, p_club: serverId(entryData.clubId) });
      state.transactions.unshift({
        type: "STAKE",
    ...
    /* Top up in dollars: $FTR at the live $FTR price, less 0.5%. */
    convertUsd: function(usdAmount){
      var rate = 1 / FTR_USD; state.wallet.usdRate = rate;
      var grossFtr = Math.round(usdAmount * rate * 100) / 100;
      var fee = Math.round(grossFtr * 0.5) / 100;
      var netFtr = Math.round((grossFtr - fee) * 100) / 100;
      state.wallet.balance += netFtr;
      state.transactions.unshift({
        type: "CONVERT",
        asset: "USD Deposit ($" + usdAmount.toLocaleString('en-US') + ")",
        shares: usdAmount,
        price: rate,
        total: netFtr,
        time: "Just now"
      });
      push('ft_convert_usd', { p_usd: usdAmount });
      save(state);
      FT.syncUI();
      window.dispatchEvent(new CustomEvent('fantrade:statechange', { detail: state }));
      return netFtr;
    },
    /* `meta` carries the bank details for a withdrawal; a transfer routes by handle. */
    sendFtr: function(amount, dest, kind, meta){
      if(amount <= 0) throw new Error("Enter an amount to send.");
      var fee = kind === 'withdraw' ? Math.max(Math.ceil(500 / FTR_USD) / 100, Math.round(amount * 0.5) / 100) : 0;   // 0.5%, at least $5
      ...
      if(kind === 'withdraw' && meta){
        push('ft_withdraw_to_bank', { p_amount: amount, p_currency: meta.currency || 'USD',
          p_holder: meta.holder, p_bank: meta.bank, p_account: meta.account, p_save: meta.save !== false });
      } else if(kind !== 'withdraw' && /^@/.test(String(dest))){
        push('ft_transfer_ftr', { p_handle: String(dest), p_amount: amount });
      }
      save(state); FT.syncUI();
      window.dispatchEvent(new CustomEvent('fantrade:statechange', { detail: state }));
    ...
    swapAssets: function(fromSym, toSym, shares, prices){
      ...
      push('ft_swap_shares', { p_from: fromSym, p_to: toSym, p_shares: shares });
      save(state); FT.syncUI();
      window.dispatchEvent(new CustomEvent('fantrade:statechange', { detail: state }));
    ...
    resetState: function(){
      state = link(JSON.parse(JSON.stringify(defaultState)));
      save(state);
      FT.syncUI();
      window.dispatchEvent(new CustomEvent('fantrade:statechange', { detail: state }));
    }
...
/* Signed in? The account is the source of truth, so take its snapshot as soon
   as the page is up, and again whenever the session changes. */
(function(){
  function sync(){
    /* FT is a binding inside this page's own script, not a global — checking
       window.FT here would silently never sync. */
    if(typeof FT === 'undefined' || !FT.syncCloud) return;
    FT.syncCloud();
    // Rank and club value are the field's answer, not this account's, so they
    // come from the board — which any page showing a rank is displaying.
    if(FT.leaderboard && document.querySelector('[data-bind="rank"],[data-bind="clubvalue"]')) FT.leaderboard(50);
  }
  if(window.FTDB){
    FTDB.ready().then(function(){ if(FTDB.signedIn()) sync(); });
    window.addEventListener('fantrade:auth', function(e){ if(e.detail) sync(); });
  }
})();

/* ── Demo market ────────────────────────────────────────────────────────
   Makes the exchange behave like a live one for demos: prices move tick
   by tick around each player's valuation, trades print, the order book
   shifts, charts extend and portfolio values follow. It all happens in this
   browser; nothing is sent to the database. It is on by default in this
   prototype; ?sim=off in the address (or FTSim.stop()) turns it off, and
   ?sim=on turns it back on. */
var FTSim = (function(){
  var KEY = 'ft_sim_v1', PREF = 'ft_sim_pref', S, on = false, timer = null, ticks = 0, feed = [], focus = null;
  var HANDLES = ['amaka_fc','tobi_eleven','gooner_jess','kwame_m','lp_scout','sofi_r','dan_okoro','priya_fc','tomh','chioma_e',
    'marcus_l','naija_baller','kopite_kel','bluesmum','ade_scores','lagos_lion','citizen_sam','zara_fc','yinka99','north_bank'];
  function gauss(){ var u = 1 - Math.random(), v = Math.random(); return Math.sqrt(-2 * Math.log(u)) * Math.cos(2 * Math.PI * v); }
  function parseVol(v){ var m = String(v || '0').match(/([\d.]+)\s*([KM]?)/i); if(!m) return 0; return Number(m[1]) * ({K:1e3, M:1e6}[m[2].toUpperCase()] || 1); }
  function fmtVol(n){ return n >= 1e6 ? (n / 1e6).toFixed(1) + 'M' : n >= 1e3 ? (n / 1e3).toFixed(1) + 'K' : String(Math.round(n)); }
  function base(a){ return a.v > 0 ? a.v / 1e7 / FTR_USD : (a._b || (a._b = a.p)); }
  function load(){ try { return JSON.parse(localStorage.getItem(KEY) || 'null'); } catch(e){ return null; } }
  function persist(){ try { localStorage.setItem(KEY, JSON.stringify(S)); } catch(e){} }
  function pref(){ var q = (location.search.match(/[?&]sim=(on|off)/) || [])[1];
    if(q){ try { localStorage.setItem(PREF, q); } catch(e){} return q === 'on'; }
    try { return localStorage.getItem(PREF) !== 'off'; } catch(e){ return true; } }
  function init(){
    var saved = load(), day = new Date().toDateString();
    S = (saved && saved.day === day && saved.a) ? saved : { day: day, a: {}, trades: [] };
    feed = S.trades || [];
    ASSETS.forEach(function(a){
      var s = S.a[a.t];
      if(!s){ var d = Number(a.d) || 0; s = S.a[a.t] = { k: 1, o: 1 / (1 + d / 100), hk: Math.max(1, (a.h || a.p) / (a.p || 1)), lk: 0, vol: parseVol(a.vol) }; s.hk = Math.max(1, s.o) * (1.003 + Math.random() * 0.004); s.lk = Math.min(1, s.o) * (0.997 - Math.random() * 0.004); }
      apply(a, s);
    });
    // A fresh day starts with the last few minutes of trading already on the tape.
    if(!feed.length){
      var now = Date.now();
      for(var i = 0; i < 48; i++){
        var a = ASSETS[Math.floor(Math.pow(Math.random(), 1.6) * ASSETS.length)];
        feed.push({ t: a.t, side: Math.random() < 0.52 ? 'buy' : 'sell', q: Math.max(1, Math.round(Math.exp(gauss() + 7.0) / Math.max(a.p, 0.0001))),
                    p: +(a.p * (1 + gauss() * 0.0015)).toFixed(6), at: now - 4000 - i * (6000 + Math.random() * 9000), who: HANDLES[i % HANDLES.length] });
      }
      S.trades = feed;
    }
  }
  function apply(a, s){
    var b = base(a);
    a.p = +(b * s.k).toFixed(6); a.d = +((s.k / s.o - 1) * 100).toFixed(2);
    a.h = +(b * s.hk).toFixed(6); a.low = +(b * s.lk).toFixed(6); a.vol = fmtVol(s.vol);
  }
  function step(){
    ticks++;
    var n = 2 + Math.floor(Math.random() * 4), changed = {}, prints = [], now = Date.now();
    var focusA = focus && Math.random() < 0.6 ? ASSETS.filter(function(x){ return x.t === focus; })[0] : null;
    for(var i = 0; i < n; i++){
      // Busier players trade more often; the player on screen trades most.
      var a = (i === 0 && focusA) || ASSETS[Math.floor(Math.pow(Math.random(), 1.6) * ASSETS.length)], s = a && S.a[a.t];
      if(!a || !s) continue;
      var sigma = a.v < 3e7 ? 0.0026 : 0.0015, pull = (1 - s.k) * 0.015 + (Math.random() < 0.5 ? -1 : 1) * 0.0002;
      var move = gauss() * sigma + pull;
      s.k = Math.max(0.5, Math.min(1.8, s.k * (1 + move)));
      s.hk = Math.max(s.hk, s.k); s.lk = Math.min(s.lk, s.k);
      var qty = Math.max(1, Math.round(Math.exp(gauss() * 1.0 + 7.0) / Math.max(a.p, 0.0001)));
      s.vol += qty; apply(a, s); changed[a.t] = move >= 0 ? 'up' : 'down';
      prints.push({ t: a.t, side: move >= 0 ? 'buy' : 'sell', q: qty, p: a.p, at: now - Math.floor(Math.random() * 900),
                    who: HANDLES[Math.floor(Math.random() * HANDLES.length)] });
    }
    feed = prints.concat(feed).slice(0, 60); S.trades = feed;
    // Portfolio values follow the market.
    syncHoldings(Object.keys(changed));
    if(ticks % 4 === 0) persist();
    window.dispatchEvent(new CustomEvent('fantrade:tick', { detail: { changed: changed, trades: prints } }));
  }
  function syncHoldings(list){
    if(typeof FT === 'undefined' || !FT.getState) return;
    var st = FT.getState(), moved = false;
    list.forEach(function(t){ var h = st.holdings && st.holdings[t], a = ASSETS.filter(function(x){ return x.t === t; })[0];
      if(h && a){ h.p = a.p; moved = true; } });
    if(moved && FT.syncUI) FT.syncUI();
  }
  function start(){ if(on) return; syncHoldings(Object.keys(S.a)); on = true; timer = setInterval(step, 1300); document.documentElement.classList.add('sim-on'); }
  function stop(){ on = false; clearInterval(timer); document.documentElement.classList.remove('sim-on'); try { localStorage.setItem(PREF, 'off'); } catch(e){} }
  init();
  window.addEventListener('fantrade:market', function(){ ASSETS.forEach(function(a){ if(S.a[a.t]) apply(a, S.a[a.t]); }); });
  if(pref()) setTimeout(start, 700);
  return {
    on: function(){ return on; }, start: function(){ try { localStorage.setItem(PREF, 'on'); } catch(e){} start(); }, stop: stop,
    focus: function(t){ focus = t; },
    trades: function(t){ return t ? feed.filter(function(x){ return x.t === t; }) : feed; },
    reset: function(){ try { localStorage.removeItem(KEY); } catch(e){} init(); }
  };
})();
/* Flash a row or number when its price moves. */
function ftFlash(el, dir){
  if(!el) return; el.classList.remove('ft-flash-up', 'ft-flash-down'); void el.offsetWidth;
  el.classList.add(dir === 'down' ? 'ft-flash-down' : 'ft-flash-up');
}
function ftAgo(ms){ var s = Math.max(1, Math.round((Date.now() - ms) / 1000)); return s < 60 ? s + 's ago' : s < 3600 ? Math.floor(s / 60) + 'm ago' : Math.floor(s / 3600) + 'h ago'; }
...
document.querySelectorAll('[data-signout]').forEach(function(b){
  b.addEventListener('click', function(e){
    ...
    var done = window.FTDB ? FTDB.signOut().catch(function(){}) : Promise.resolve();
    setTimeout(function(){ done.then(function(){ FT.signOut(); }); }, 700);
  });
});
...
JS_SHELL = JS_SHELL.replace('__VERIFIED_PHOTOS__', _verified_photos())
