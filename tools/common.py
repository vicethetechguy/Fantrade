...
var FT = (function(){
  ...
  var defaultState = {
    ...
    wallet: {
      ...
      locked: 0,
      seasonEarned: 19640,
      gbpRate: 0.6686   // £1 = $1.3372 = 0.6686 $FTR at $2
    },
    holdings: {
    ...
    transactions: [
      ...
      { type: "CONVERT", asset: "GBP Deposit (£1,000)", shares: 1000, price: 0.6686, total: 665.26, time: "Sep 07, 11:45" }
    ]
  };
  ...
  function link(st){
    ...
    if(!st.econV2){
      ...
      if(st.wallet){ st.wallet.gbpRate = 0.6686; }
      st.econV2 = true;
    }
    delete st.clubs;
  ...
  function applySnapshot(snap){
    ...
    if(snap.wallet){
      ...
      state.wallet.gbpRate = Number(snap.wallet.gbp_rate) || state.wallet.gbpRate;
    }
    if(snap.holdings){
      var held = {};
  ...
  return {
    ...
    convertGbp: function(gbpAmount){
      ...
      push('ft_convert_gbp', { p_gbp: gbpAmount });
      save(state);
      FT.syncUI();
      window.dispatchEvent(new CustomEvent('fantrade:statechange', { detail: state }));
      return netFtr;
    ...
    sendFtr: function(amount, dest, kind, meta){
      ...
      if(kind === 'withdraw' && meta){
        push('ft_withdraw_to_bank', { p_amount: amount, p_currency: meta.currency || 'GBP',
